...
data <- read_excel(file_name, sheet = sheet) %>%
      select(c(RatID, `BW (pre)`, `BW (post)`, `Measurement 1`, `Measurement 3: 2-3H after treatment 2`, `MP 1.1`, `MP 1.2`, `MP 1.3`, `MP 3.1`, `MP 3.2`, `MP 3.3`)) %>% 
      select(c(RatID, `BW (pre)`, `BW (post)`, `Measurement 1`, `Measurement 3: 2-3H after treatment 2`, `MP 1.1`, `MP 1.2`, `MP 1.3`, `MP 3.1`, `MP 3.2`, `MP 3.3`)) %>% 
    rename(
    ...
    rename_with(
      ~ sub("^MP\\s*(\\d+)\\.(\\d+)$", "PA\\1.\\2", .x),
      .cols = matches("^MP\\s*(1|3)\\.(1|2|3)$")
    ) %>% 
