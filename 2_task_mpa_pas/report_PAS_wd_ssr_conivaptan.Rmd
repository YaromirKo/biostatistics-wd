---
title: "PAS Measurement 1 (24H WD) vs Measurement 3 (SSR, Conivaptan) (hand, machine)"
...
data <- read_excel(file_name, sheet = sheet) %>%
  ...
  filter(
    Measurement.3 %in% c("SSR", "Conivaptan"),
    Trt %in% c("SSR", "Conivaptan") | (Trt == "24H WD" & MPA >= threshold)
  ) %>% 
  mutate(
    Trt = factor(Trt, levels = c("24H WD", "SSR", "Conivaptan"))
  ) %>%
...
# (Hand) Analyse 24 WD vs SSR vs Conivaptan
## PAS: Student Model
...
myname <- paste0(myname0, "_wd_ssr_conivaptan_pas_Hand")
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "wd_ssr_conivaptan_pas_check_prior.rds"),
...
plt[["WD&SSR&Conivaptan PAS prior vs data (hand)"]] <- plot(cond_eff, 
                                       points = TRUE, 
...
set.seed(2)
plt[["WD&SSR&Conivaptan PAS prior pp_check (hand)"]] <- pp_check(std.prior, ndraws = 11)

print(cond_eff[[1]])
print(plt[["WD&SSR&Conivaptan PAS prior vs data (hand)"]])
print(plt[["WD&SSR&Conivaptan PAS prior pp_check (hand)"]])
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "wd_ssr_conivaptan_pas_model_summary.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
plt[["WD&SSR&Conivaptan PAS pp_check (hand)"]] <- pp_check(std.fit, ndraws = 21)
print(plt[["WD&SSR&Conivaptan PAS pp_check (hand)"]])
...
plt[["WD&SSR&Conivaptan PAS ac (hand)"]] <- stan_ac(std.fit$fit)
print(plt[["WD&SSR&Conivaptan PAS ac (hand)"]])
...
plt[["WD&SSR&Conivaptan PAS emm (hand)"]] <- emm %>% emm_inorder %>%
  ...
  # geom_vline(xintercept=-threshold, linetype="dashed", color="black") +
  labs(title=myname, y="", x="PAS (mm) WD&SSR&Conivaptan")

print(plt[["WD&SSR&Conivaptan PAS emm (hand)"]])
...
plt[["WD&SSR&Conivaptan PAS emm (hand) with Data Points"]] <- 
  plt[["WD&SSR&Conivaptan PAS emm (hand)"]] +
    geom_point(
      pch = 4,
...
print(plt[["WD&SSR&Conivaptan PAS emm (hand) with Data Points"]])
...
plt[["WD&SSR&Conivaptan PAS (hand)"]] <- emmc %>% emm_inorder %>%
...
print(plt[["WD&SSR&Conivaptan PAS (hand)"]])

```

# (Machine) Analyse 24H WD vs SSR vs Conivapta
## PAS: Student Model
...
myname <- paste0(myname0, "_wd_ssr_conivaptan_pas_MP")
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "wd_ssr_conivaptan_pas_check_prior_machine.rds"),
  file_refit = "on_change"
)
...
plt[["WD&SSR&Conivaptan PAS prior vs data (Machine)"]] <- plot(cond_eff, 
                                       points = TRUE, 
                                       point_args = list(width = 0.1, shape="o", col="red"),
                                       plot=FALSE)[[1]] + 
                                  geom_hline(yintercept=0, linetype="dashed", color="black")
...
set.seed(2)
plt[["WD&SSR&Conivaptan PAS prior pp_check (Machine)"]] <- pp_check(std.prior, ndraws = 11)

print(cond_eff[[1]])
print(plt[["WD&SSR&Conivaptan PAS prior vs data (Machine)"]])
print(plt[["WD&SSR&Conivaptan PAS prior pp_check (Machine)"]])
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "wd_ssr_conivaptan_pas_model_summary_machine.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
plt[["WD&SSR&Conivaptan PAS pp_check (Machine)"]] <- pp_check(std.fit, ndraws = 21)
print(plt[["WD&SSR&Conivaptan PAS pp_check (Machine)"]])
...
plt[["WD&SSR&Conivaptan PAS ac (Machine)"]] <- stan_ac(std.fit$fit)
print(plt[["WD&SSR&Conivaptan PAS ac (Machine)"]])
...
plt[["WD&SSR&Conivaptan PAS emm (Machine)"]] <- emm %>% emm_inorder %>%
  ggplot(aes(y = fct_rev(interaction(Trt, sep=", ", drop=TRUE)), x = .value, fill = Trt)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  # geom_vline(xintercept=threshold, linetype="dashed", color="black") +
...
print(plt[["WD&SSR&Conivaptan PAS emm (Machine)"]])
...
plt[["WD&SSR&Conivaptan PAS emm (Machine) with Data Points"]] <- 
  plt[["WD&SSR&Conivaptan PAS emm (Machine)"]] +
    geom_point(
      pch = 4,
...
print(plt[["WD&SSR&Conivaptan PAS emm (Machine) with Data Points"]])
...
plt[["WD&SSR&Conivaptan PAS (Machine)"]] <- emmc %>% emm_inorder %>%
  ggplot(aes( y = fct_rev(interaction(contrast, sep=", ", drop=TRUE)), x=.value, fill=contrast)) %>%
  my.stat_eyeh() +
  geom_vline(xintercept=0, linetype="dashed", color="black") +
  labs(title=myname, y="", x="PAS in mm, contrast: WD vs Control")

print(plt[["WD&SSR&Conivaptan PAS (Machine)"]])
