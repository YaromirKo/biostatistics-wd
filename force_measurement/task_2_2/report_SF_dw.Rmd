...
```{r dta_pre}
trt_keep <- c("SSR", "Conivaptan")

rats_keep <- dta %>%
  filter(Trt %in% trt_keep, Time == "2_3H") %>%
  distinct(RatID)

dta_pre <- dta %>%
  filter(
    Trt %in% trt_keep |
      (Time == "24H" & RatID %in% rats_keep$RatID)
  ) %>%
  droplevels()
```
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "dw_check_prior_task_2_2.rds"),
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "dw_model_summary_task_2_2.rds"),
                   file_refit = "on_change")
summary(std.fit)
