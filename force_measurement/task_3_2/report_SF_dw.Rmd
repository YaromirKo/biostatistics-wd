---
title: "SF, LR dW DW | WD vs Sp vs Control 2/3   groups: Bayesian regression of smoothed stretch forces"
...
# Analyse WD vs Sp vs Control 2/3

```{r dta_pre}
trt_keep <- c("Sp", "Control 2", "Control 3")
...
dta_pre <- dta %>%
  filter(
    Trt %in% trt_keep |
      (Time == "24H" & RatID %in% rats_keep$RatID & Trt != "Control 1")
  ) %>%
  droplevels()
```
...
myname <- "SF 0-2 span=0.4 (WD vs Sp vs Control 2/3)"
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "dw_check_prior_task_3_2.rds"),
...
std.fit <-  update(std.prior,
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "dw_model_summary_task_3_2.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
myname <- "SF span=0.4 0-2 CI dW WD vs Sp vs Control 2-3"
