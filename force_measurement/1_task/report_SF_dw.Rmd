---
title: "SF, LR dW DW | 24H WD vs Control 1 groups: Bayesian regression of smoothed stretch forces"
...
myname <- "SF 0-2 span=0.4 (Saline, TRH, GnRH, CCK8) Time"

plt[["Groups, Time"]] <- 
  dta_pre %>% 
  group_by(Group, Time, dT) %>% 
  median_hdci(W, .width = 0.95) %>%
  ungroup() %>%
  ggplot(aes(x=interaction(dT, Time, sep=", ", drop=TRUE), y=W, fill=Group)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=.lower, ymax=.upper),
                  width=.2,   # Width of the error bars
                  position=position_dodge(.9)) +
...
std.prior <- brm(
  ...
  iter = 4e4,
  sample_prior = "only",
  file = paste0(rds_folder_name, "dw_check_prior.rds"),
...
cond_eff <- conditional_effects(std.prior,
                                "Group",
                                method="fitted",
                                conditions=make_conditions(dta_pre, vars = c("Time"))
                                )

plt[["Groups Time prior check"]] <- plot(cond_eff,
                                       points = TRUE,
...
set.seed(2)
plt[["Groups Time prior pp_check"]] <- pp_check(std.prior, ndraws = 11)

print(cond_eff[[1]])
print(plt[["Groups Time prior check"]])
print(plt[["Groups Time prior pp_check"]])
...
std.fit <-  update(std.prior, 
                   ...
                   seed = my.seed, 
                   cores = my.cores,
                   file = paste0(rds_folder_name, "dw_model_summary.rds"),
                   file_refit = "on_change")
summary(std.fit)
...
## SF $\Delta W$: Contrasts M2 vs M3,4,5:

Contrasts Groups Time as a table:
...
plt[["SF by Time"]] <- emmc %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, Group, sep=", ", drop=TRUE), y = .value, fill = Group)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, contrast: M2 vs M3/4/5")
...
plt[["SF by Group"]] <- filtered_emmc %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, Time, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, Group")
...
plt[["SF by Group 0"]] <- filtered_emmc_g0 %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, Time, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, Group")
...
plt[["SF by Group 1"]] <- filtered_emmc_g1 %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, Time, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, Group")
...
plt[["SF by Group 6"]] <- filtered_emmc_g5 %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, Time, sep=", ", drop=TRUE), y = .value, fill = Time)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, Group")
...
plt[["SF by Group 7"]] <- filtered_emmc_g1 %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(contrast, Time, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, Group")
...
plt[["SF by Group contrast of contrasts"]] <- filtered_emmc %>% gather_emmeans_draws %>%
...
plt[["SF by Group 7 new"]] <- filtered_emmc_g1 %>% gather_emmeans_draws %>%
  ggplot(aes(x = interaction(Time, contrast, sep=", ", drop=TRUE), y = .value, fill = contrast)) %>%
  my.stat_eye() +
  geom_hline(yintercept=0, linetype="dashed", color="black") +
  labs(title=myname, x="0-2 sec", y="dW, Group")
...
myname <- "SF span=0.4 0-2 CI dW CherryRat TG 8 groups"
